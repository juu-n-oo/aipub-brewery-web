...
const buttonVariants = cva(
  "inline-flex items-center cursor-pointer justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 aria-invalid:border-destructive",
  {
    variants: {
      ...
      size: {
        default: 'h-9 px-4 py-2 has-[>svg]:px-3 [&.has-svg-fallback]:px-3',
        sm: 'h-8 rounded-md gap-1.5 px-3 has-[>svg]:px-2.5 [&.has-svg-fallback]:px-2.5',
        lg: 'h-10 rounded-md px-6 has-[>svg]:px-4 [&.has-svg-fallback]:px-4',
        icon: 'size-9',
      },
    },
    defaultVariants: {
...
function Button({
  ...
  useEffect(() => {
    ...
    const hasSvgChild = Array.from(buttonRef.current.children).some(
      (child) => child.tagName.toLowerCase() === 'svg',
    );
    if (hasSvgChild) {
      buttonRef.current.classList.add('has-svg-fallback');
    } else {
      buttonRef.current.classList.remove('has-svg-fallback');
    }
  }, [children, loading]);
